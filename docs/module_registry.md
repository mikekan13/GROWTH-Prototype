...
| SpellGrantService | `services/spell-grant.ts` | Teaches an authored ForgeItem('spell') to a character: enforces mechanics completeness (dr+manaCost), derives pillar from primary school, derives requiresSystemReview from config, writes magic.<pillar>.knownSpells, dedupes, broadcasts. GM/ADMIN-only (player never self-teaches). NO KRMA movement (spell-learning pricing = pending ruling, NEEDS-MIKE) | forge, economy-config, campaign-stream, Prisma |
...
| /api/characters/[id]/spells | POST | SpellGrantService (GM teaches an authored spell ForgeItem — confirm step of the player→GM→godhead Woven pipeline, r-2026-07-22-01 #5) |
