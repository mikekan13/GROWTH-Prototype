...
## Text generation — RunPod Serverless local lane (2026-08-23)
Endpoint `growth-text` (`y0lkvhwp0xkzrs`, worker-v1-vllm v2.25.2, template `jh3qgfbp9o`, volume o5y6of5tje, scale-to-zero, flashboot, H100/A100-80 class). Serves `/openai/v1/chat/completions` as model `growth-local` — currently Qwen3.6-27B-abliterated FP8 from `/runpod-volume/models/qwen36-27b-abliterated` (32k ctx pin, hermes tool parser). Qwen3.8-27B-abliterated (BF16, 55.6GB) downloaded to `/runpod-volume/models/qwen38-27b-abliterated` as the fine-tune base + smoke-test candidate. The retired H100 pod's DAYA-vLLM role lives here now; volume resized 200→280GB.
