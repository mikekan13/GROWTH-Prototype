...
## Text generation — RunPod Serverless local lane (LIVE 2026-08-24)
Endpoint `growth-text` (`y0lkvhwp0xkzrs`, worker-v1-vllm **v2.24.0**, template `jh3qgfbp9o`, volume o5y6of5tje, scale-to-zero, flashboot, H100/A100-80/H200 class, **`minCudaVersion: "13.0"`** — required: recent worker images are CUDA-13 builds that crash on older-driver hosts). Serves `/openai/v1/chat/completions` as model `growth-local` — Qwen3.6-27B-abliterated, FP8 on-load + enforce-eager + gpu-util 0.90 (the proven July pod recipe; BF16-unquantized OOMs at init). First serve verified 2026-08-24, **warm latency ~1.3s**. Cold-start-safe call pattern: async `POST /v2/{id}/run` with `{input:{openai_route,openai_input}}` + poll `/status/{id}` (sync HTTP drops ~180s). Qwen3.8-27B-abliterated (BF16, 55.6GB, download verified) staged at `/runpod-volume/models/qwen38-27b-abliterated` as fine-tune base + smoke-test candidate (needs a ≥Aug-13 vLLM worker tag). The retired H100 pod's DAYA-vLLM role lives here now; volume resized 200→280GB.
