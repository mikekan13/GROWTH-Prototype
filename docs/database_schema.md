...
- WP8 (mechanics coupling, `daya/mechanics/thorns.ts`): no schema change, but `personaProfile` now also carries an `activeThornBlocks` key (array of `{subjectPattern, mode, strength}` — the WP4 `ThornBlock` shape) — the entity's currently-active Thorn-fired recall blocks, persisted here (same free-form-JSON reuse pattern WP10 used on `DayaMemoryEntry.classification`), loaded by `daya/ensemble.ts` on every stimulus and fed to `daya/recall.ts` instead of an empty array.
...
- WP10 (`daya/dream.ts`) still no schema change, but extends the informal keys living inside the existing `classification` JSON blob (already free-form, no migration needed): `labileUntilNextDream` (set by `daya/recall.ts` on every surfaced/rehearsed row — T0 reconsolidation window, cleared by the next dream tick that processes the memory), `lastTouchedTick` (spacing-effect bookkeeping), and, on a cluster's anchor row only, `ruminationLock`/`ruminationTicks`/`suppressionStrength`/`thornProposed` (trauma-loop state — Phase 1 has no dedicated Thorn table, so a locked/deepened cluster crossing a tick threshold only *logs* a Thorn PROPOSAL as a zero-cost `DayaModelCall` audit row, subsystem `thorn_proposal`, never auto-creates one). A `source:'dream'` row with `classification.kind==='tick_marker'` (written by `daya/scheduler.ts`'s dreamTickHandler) is a pure bookkeeping marker, excluded from clustering/retag/maintenance entirely — it is never phenomenal content and never recalled as the entity's own.

### DayaAffect
1:1 with DayaEntity. Mood vector — event-driven internal state for AI-controlled characters (absorbs the uncommitted `CharacterDisposition` prototype). Drives move ONLY on real game events (frequency loss, death saves, goal outcomes, advancement, WP10 dream-tick residue via a `dream_consolidation` DispositionEvent, WP8 mechanics coupling via `pool_spent`/`pool_restored`/`thorn_fired` DispositionEvents — effort/damage actually spending a pool, rest actually restoring one, a Thorn actually firing) and decay toward baseline on the campaign clock. Read by prompt assembly (JEWL table state, npc_speak) so an AI-voiced character's condition shapes behavior. See `services/daya-affect.ts`.
