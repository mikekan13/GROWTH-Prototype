...
- **AI Network Infrastructure (2026-08-23)** — `ai/network/` routing layer: one chokepoint for every AI call (lanes judgment/classify/grunt/godhead/local; fail-closed privacy wall; INV-119 sanitize seam), unified `AiCall` metering ledger (cache stats + per-campaign cost attribution, report via `scripts/ai-costs.ts`), fine-tune trace capture (dev-era JEWL dispatches = distillation corpus for the GROWTH-native local model). Serverless text endpoint `growth-text` (`y0lkvhwp0xkzrs`, vLLM, scale-to-zero) serving Qwen3.6-27B-abliterated from the volume; Qwen3.8-27B-abliterated staged as fine-tune base. H100 pod retired; volume 280GB. Design: `AI-NETWORK-DESIGN-INTENT-2026-08-23.md` + `AI-ECONOMICS-2026-08-23.md`. NEXT: smoke-test 3.8 vs 3.6, flip grunt/local lanes live, GROWTH fine-tune.
