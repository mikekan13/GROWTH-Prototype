...
function deepCloneCharacter(c: GrowthCharacter): GrowthCharacter {
  // structuredClone handles undefined / circular refs more gracefully than JSON round-trip,
  // and gives a specific error instead of a cryptic "Unexpected end of JSON input".
  if (typeof structuredClone === 'function') {
    try { return structuredClone(c); } catch { /* fall through */ }
  }
  const s = JSON.stringify(c);
  if (!s) throw new Error('deepCloneCharacter: character serialized to empty/undefined');
  return JSON.parse(s);
}
