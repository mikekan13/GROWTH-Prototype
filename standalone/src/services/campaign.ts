...
export async function updateCampaign(
  ...
  // Merge matureContent into aiSettings JSON
  let aiSettingsUpdate: Record<string, unknown> | undefined;
  if (input.matureContent !== undefined) {
    const existing = campaign.aiSettings ? JSON.parse(campaign.aiSettings) : {};
    existing.matureContent = input.matureContent;
    aiSettingsUpdate = existing;
  }
  ...
  const updated = await prisma.campaign.update({
    ...
    data: {
      ...
      ...(aiSettingsUpdate !== undefined && { aiSettings: JSON.stringify(aiSettingsUpdate) }),
    },
  });
