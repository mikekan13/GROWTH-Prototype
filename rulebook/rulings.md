...
- **Implementation note (flag, not yet Mike-confirmed)**: advancement is modeled as a **pure sheet mutation** — reduce Max Frequency by the cost + apply the +1 — mirroring the existing ledger-free `setAttributeLevel`/`updateSkillLevel`. It does **NOT** credit the character wallet (that is the *generic* Frequency-Spend op, a different action). Whether the consumed Frequency should also move on the KRMA ledger (lock/crystallize into the stat) is OPEN — deferred to the crystallization gate. [NEEDS MIKE if the ledger side matters before beta.]
