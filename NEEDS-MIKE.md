...
- **Gap 1 — Spend doesn't apply the upgrade (mechanics call).** Spec acceptance = "spend 1 → max−1 AND **target stat +1**." Today Spend only reduces max and credits 1 KRMA to the character wallet; there's **no target picker** and nothing raises a chosen attribute/skill. Two readings and I won't guess: **(a)** Spend is one atomic action that both drops max −1 AND bumps a picked attribute/skill +1 (needs a target picker wired to the advancement path); **(b)** Spend just converts max-Freq → character-wallet KRMA, and a **separate** advancement UI later spends that KRMA to raise stats (two steps; current behavior is already half of this). Which model?
