...
4. **Magic advancement / full casting** — ✅ DESIGN RULED 2026-07-22 (r-2026-07-22-01): build full casting, not a stub. Locked: DR-50+ = system/Terminal oversight threshold; mana↔KRMA is a FIXED system rate (≈4:1, **exact # TBD**); JEWL co-pilots ALL casting; spells = freeform authored objects w/ predefined fields (like nectars/thorns, can emit permanent effects); Woven approval routes player→GM→godhead. Schema ✅ SIGNED OFF 2026-07-23 (r-2026-07-23-01). Foundation shipped (magic-cast engine + tunable config + spell schema, `828acb0`). **Still needs Mike (playtest-gated, not blocking the build):** (a) exact mana↔KRMA number; (b) per-effect DR value review (Mike: "DR is a detailed thing we should discuss"). Build follow-ons listed in STATE-OF-PLAY.
   **Cast route SHIPPED 2026-07-23** (`services/magic-cast-ops.ts` + POST `/api/characters/[id]/cast`, e2e 17/17). Three canon-SILENT calls I defaulted — veto if wrong: (c) **mana is consumed on a failed cast** (no refund rule exists anywhere; Mana_System.md never distinguishes success/fail); (d) **DR ≥ systemEngagementDR FLAGS system review but does not BLOCK resolution** — matches the signed-off schema's `requiresSystemReview` boolean; if oversight must gate the roll itself, say so and I'll hold the cast pending godhead review instead; (e) **mana regen trigger** — conversion rate (≈4:1) is validated but HOW the pool refills (rest? KRMA purchase only? both?) is #needs-validation flavor; the cast route only deducts, nothing refills mana yet. Also still open from r-2026-07-15-01: (f) **trainable-schools storage** — wild-fail marks are reported in the cast result but NOT persisted; proposal: `magic.<pillar>.trainableSchools: MagicSchool[]` mirroring skillLevels — confirm and I'll wire advancement cost 2.
   **Woven pipeline SHIPPED 2026-07-23** (spell forge type + player request → GM approve → godhead-authorable mechanics → learnSpell → knownSpells → castable; test-spell-pipeline 9/9). One deliberate divergence from the T32 mirror — veto if wrong: (g) **learning a spell moves NO KRMA and carries NO KV.** The nectar golden path pays godhead→character because bestowal is a REWARD; a player-REQUESTED spell isn't, and no ruling prices spell-learning (mana is system-priced at cast time instead; the evaluator doesn't price knownSpells today). If spells should carry KV at learn time (TKV impact + a ledger transfer, and if so who pays whom), say so and I'll add it at the learnSpell step.
